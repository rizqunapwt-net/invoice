...
<!-- Modal -->
<div class="modal fade" id="editpretes{{$data->id}}" tabindex="-1" role="dialog" aria-labelledby="editsoalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
        ...
        <div class="modal-body">
            <form method="post" action={{route('updatesoalpretes')}}>
                @csrf
                <div class="form-group">
                  <label for="exampleInputEmail1">Soal </label>
...
<div class="modal fade" id="tambahpilihan{{$data->id}}" tabindex="-1" role="dialog" aria-labelledby="tambahsahpilihanLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
            <form method="post" action={{route('simpanpilihanpretes')}}>
                @csrf
                <div class="form-group">
                 ...
                 <input type="text" name="pilihan" class="form-control"  > 
                  ...
                  <input type="hidden" name="seminar_id" value="{{$seminar->id}}">
                  <input type="hidden" name="pretes_id" value="{{$data->id}}">     
        </div>
        <div class="modal-footer">
...
<div class="modal fade" id="editpilihan{{$pil->id}}" tabindex="-1" role="dialog" aria-labelledby="editpilihanLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
            <form method="post" action={{route('updatepilihanpretes')}}>
                @csrf
                <div class="form-group">
                  <label for="exampleInputEmail1">Pilihan </label>
        ...
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                                      ...
                                      <li >
                                        {{$pil->pilihan}}                                        <form action="{{route('pilihanpretes.hapus',$pil->id)}}" method="POST">
                                   ...
                                   {{-- @foreach($data->pilihanpretes->pilihan as $pilihan)
                                   ...
                                   @endforeach --}}
                                    ...
                                    <td>
                                        ...
                                        <form action="{{route('pretes.hapus',$data->id)}}" method="POST">
                                            <a href="#" class='btn kecil btn-warning btn-sm' data-toggle="modal" data-target="#editpretes{{$data->id}}"><i class="far fa-edit"></i></a>
                                            @csrf
                                            @method('delete')
